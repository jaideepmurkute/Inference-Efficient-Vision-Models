...
def create_model(cfg, device, logger):
    try:
        if getattr(cfg, "use_timm", True):
            model = timm.create_model(
                cfg.model_name, pretrained=cfg.pretrained, num_classes=cfg.num_classes
            )
        else:
            ...
            # Replace imagenet Head for custom classification
            # ResNet: model.fc
            ...
            if hasattr(model, "fc"):
                ...
                if isinstance(model.classifier, nn.Sequential):
                    ...
                    logger.warning(
                        f"Could not automatically find classifier head for {cfg.model_name}. You may need manual adjustment."
                    )
