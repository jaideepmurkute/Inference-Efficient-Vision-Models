...
class StructuredPruningEngine:
    ...
    def get_pruner(self, model, example_inputs):
        ...
        if method == 'random':
            ...
            # "Group Norm" in pruning usually refers to L2 magnitude of the group.
            # torch-pruning MagnitudeImportance(p=2) handles this.
            imp = tp.importance.MagnitudeImportance(p=2)
        elif method == 'taylor':
        ...
        # For ResNet/CNNs, we generally want to protect the final classification head
        # to ensure the number of classes remains constant.
        ignored_layers = []
        ...
        # 2. Protect specific layers if named 'fc' or similar (common in ResNet)
        # torch-pruning dependency graph usually handles connections automatically.
        
        self.logger.info(f"Protected {len(ignored_layers)} layers (Head). Params: Method={method}, Round={self.cfg.round_to}")
    ...
    def finetune(self, model, train_loader, val_loader, epochs, learning_rate):
        """
        Standard fine-tuning loop with Best Model Restoration.
        """
        ...
        for epoch in range(epochs):
            ...
            # Using enumerate loop structure preferred by user
