...
def create_model(model_name, use_timm, pretrained, num_classes, logger):
    ...
    if use_timm:
        ...
        if hasattr(model, "fc"):
            ...
            logger.warning(
                f"Could not automatically find classifier head for {model_name}."
            )
    return model
...
def load_checkpoint(
    ...
    assert load_type in valid_load_types, (
        f"Invalid load_type. Must be one of {valid_load_types}"
    )
    ...
    if load_type == "model":
        assert model is not None, (
            "Model object must be provided to load model checkpoint into."
        )
        load_path = os.path.join(ckpt_dir, f"{prefix}model_{suffix}.pth")
        try:
    ...
    if load_type == "optimizer":
        assert optimizer is not None, (
            "Optimizer object must be provided to load optimizer checkpoint into."
        )
        load_path = os.path.join(ckpt_dir, f"{prefix}optimizer_{suffix}.pth")
        try:
    ...
    if load_type == "scheduler":
        assert scheduler is not None, (
            "Scheduler object must be provided to load scheduler checkpoint into."
        )
        load_path = os.path.join(ckpt_dir, f"{prefix}scheduler_{suffix}.pth")
        try:
    ...
    if load_type == "training_log":
        load_path = os.path.join(ckpt_dir, f"{prefix}training_log.json")
        try:
            with open(load_path, "r") as f:
